...
- 👥 **Команда (workspaces)** — участники с ролями
  ...
  фокус-сессии, мини-дашборд (кто трекает/недобор/перегруз), журнал действий
  (аудит-лог), смена ролей владельцем.
...
- 🧾 **Аккаунт** — смена имени/email (сброс верификации), верификация почты
  письмом (токен, TTL 24ч), смена пароля, 2FA (TOTP, QR).
- ✅ **Задачи++** — исполнитель, приоритет (высокий/средний/низкий), дедлайн,
  ...
  задач (чипы + «В шаблон»).
- 💰 **Деньги** — ставка ₽/ч и недельный бюджет часов у проекта, billable-флаг
  (тумблер ₽) и комментарий у записи, суммы в отчётах, превью счёта
  (billable-часы × ставка) с печатью в PDF.
- 📋 **Таймшиты** — участник отправляет неделю на проверку; админ утверждает /
  возвращает с комментарием (+массовое утверждение); утверждённая неделя
  блокирует записи.
- 🔗 **Публичная ссылка на отчёт** — токен-страница `/r/<token>` c опциями
  «скрыть суммы/имена» и выключателем.
- 🔔 **Центр уведомлений** — назначенные задачи, утверждение/возврат
  таймшитов; колокольчик в шапке.
...
Верификация и приглашения уходят через SMTP (`SMTP_HOST`, `SMTP_PORT`,
`SMTP_USER`, `SMTP_PASS`, `MAIL_FROM`); без настройки SMTP письма
логируются в консоль сервера (dev-режим). `FRONTEND_URL` — база для ссылок
в письмах (по умолчанию `http://localhost:3000`).
